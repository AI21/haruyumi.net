 ...
 * return
 */
function form_event_regist($eventClassId, $areaGroupList, $eventNameList, $syubetsuList, $kaijoList, $uploadFileNum) {
	$html = '';
	...
	$addEmptyFlg = true;
	if ($eventNameList['numRows'] === 1) {
		$addEmptyFlg = false;
	}
	$html .= admin_form_dropdown($eventNameList['result'], 'event-name-id', '', 'event_name_id', 'event_name', '', $addEmptyFlg);
	$html .= '</dd>';
	// 審査日・審査種別
	$html .= '<dt>審査日・種別</dt>';
	$html .= '<dd>';
	$html .= '<ul class="mb-2" id="event-date-tab" role="tablist">';
	if ($eventClassId === event_CLASS_ID_CHUOU) {
		// 中央審査の場合は3日分の審査日を表示
		for ($i = 0; $i < 3; $i++) {
			$html .= '<li class="nav-item mb-2" role="presentation">';
			$html .= $i + 1 . '日目';
			// $data = $eventDetail['date_holder_grade']['result'][$i] ?? null;
			// if (empty($data) === false) {
			// 	$html .= admin_form_date("event-date-" . ($i + 1), $data['event_date'], "event-date-" . ($i + 1), 'ms-2');
			...
			// } else {
				$html .= admin_form_date("event-date-" . ($i + 1), "", "event-date-" . ($i + 1), "ms-2");
				$addEmptyFlg = true;
		...
		$html .= '<li class="nav-item mb-2" role="presentation">';
		$data = $eventDetail['date_holder_grade']['result'][0] ?? null;
		$addEmptyFlg = true;
		// 種別が1件の場合、もしくは地方審査の場合は空選択なしとする
		if ($syubetsuList['numRows'] === 1 || $eventClassId === event_CLASS_ID_CHIHO) {
			$addEmptyFlg = false;
		}
		if (empty($data) === false) {
			$html .= admin_form_date("event-date-1", $data['event_date'], "event-date-1");
	...
	for ($i = 0; $i < 3; $i++) {
		$kaijoId = NULL;
		$additionalInfo = NULL;
		if (empty($eventDetail['kaijo_list']) === false) {
			if (empty($eventDetail['kaijo_list']['result'][$i]['kaijo_id']) === false) {
				$kaijoId = $eventDetail['kaijo_list']['result'][$i]['kaijo_id'];
			}
			if (empty($eventDetail['kaijo_list']['result'][$i]['additional_info']) === false) {
				$additionalInfo = $eventDetail['kaijo_list']['result'][$i]['additional_info'];
			}
	...
	$class = '';
	$label = '設定';
	if ($eventClassId === event_CLASS_ID_RENGO) {
	...
	// if ($eventDetail['event_class_id'] == event_CLASS_ID_RENGO) {
	// 	// 連合審査の場合は審査日の3ヶ月前
	// 	$webApplyDescription = '審査日の3ヶ月前';
	// 	$webApplyStartDay = date('Y/m/d', strtotime($eventDetail['date_holder_grade']['result'][0]['event_date'] . ' -3 month'));
	// 	$week = ['日', '月', '火', '水', '木', '金', '土'];
	// 	$webApplyStartDay .= '（' . $week[date('w', strtotime($webApplyStartDay))] . '）';
	// } else {
	// 	if (!empty($eventDetail['uketuke_limit_aikyuren_st'])) {
	// 		$webApplyStartDay = date('Y/m/d', strtotime($eventDetail['uketuke_limit_aikyuren_st'] . ' -1 week'));
	// 		$week = ['日', '月', '火', '水', '木', '金', '土'];
	// 		$webApplyStartDay .= '（' . $week[date('w', strtotime($webApplyStartDay))] . '）';
	// 	}
	// }
	...
	$html .= '<dt>添付資料</dt>';
	$html .= '<dd>';
	$html .= form_file_document('event', 'event-files', '', $uploadFileNum);
	$html .= '</dd>';

	$html .= admin_form_hidden("event-id", 0);
	$html .= admin_form_hidden("event-class-id", $eventClassId);
...
function form_event_revision($eventDetail, $uploadFileNum) {
	...
	// 審査名
	$eventName = '';
	if (empty($eventDetail['area_group_name']) === false) {
		$eventName = '【' . $eventDetail['area_group_name'] . '】';
	}
	...
	// 審査名
	$html .= '<dt>審査名</dt>';
	$html .= '<dd>' . $eventName . '</dd>';
	// 審査日・審査種別
	$html .= '<dt>審査日・種別</dt>';
	$html .= '<dd>';
	$html .= '<ul class="mb-2" id="event-date-tab" role="tablist">';
	if ($eventDetail['event_class_id'] === event_CLASS_ID_CHUOU) {
		// 中央審査の場合は3日分の審査日を表示
		for ($i = 0; $i < 3; $i++) {
			$html .= '<li class="nav-item mb-2" role="presentation">';
			$html .= $i + 1 . '日目';
		...
		$data = $eventDetail['date_holder_grade']['result'][0] ?? null;
		if (empty($data) === false) {
			$html .= admin_form_date("event-date-1", $data['event_date'], "event-date-1", 'ms-2');
			...
			$html .= admin_form_date("event-date-1", "", "event-date-1", "ms-2");
	...
	if ($eventDetail['event_class_id'] === event_CLASS_ID_CHUOU) {
		...
		$html .= '<dd>';
		$html .= '<p class="mt-1 mb-0">';
		$html .= admin_form_dropdown($syubetsuList['result'], 'all-holder-grade-id', '', 'holder_grade_id', 'holder_grade_name', $eventDetail['all_holder_grade_id'], true);
		$html .= '</p>';
		$html .= '</dd>';
	} else {
		$html .= admin_form_hidden("all-holder-grade-id", $eventDetail['all_holder_grade_id']);
	}
	// 会場（第1〜3会場）
	$html .= '<dt>会場</dt>';
	$html .= '<dd>';
	...
	if ($eventDetail['event_class_id'] === event_CLASS_ID_CHUOU) {
		$html .= '<dt>全弓連締切日</dt>';
		$html .= '<dd>';
		$html .= admin_form_date("uketuke-limit-zenkyuren", $eventDetail['uketuke_limit_zenkyuren'], "uketuke-limit-zenkyuren");
		$html .= '</dd>';
	} else {
	...
	$class = 'd-none';
	$label = '未定';
	if (empty($eventDetail['uketuke_limit_aikyuren_st']) === false or empty($eventDetail['uketuke_limit_aikyuren_ed']) === false) {
		$checked = ' checked="checked"';
		$class = '';
		$label = '設定';
	}
	...
	$html .= '</span>';
	$html .= '<span id="uketuke-limit-aikyuren-area" class="' . $class . '">';
	$html .= admin_form_date("uketuke-limit-aikyuren-st", $eventDetail['uketuke_limit_aikyuren_st'], "uketuke-limit-aikyuren-st");
	$html .= ' ～ ';
	$html .= admin_form_date("uketuke-limit-aikyuren-ed", $eventDetail['uketuke_limit_aikyuren_ed'], "uketuke-limit-aikyuren-ed");
	$html .= '</span>';
	$html .= '</dd>';
	// WEB申込開始日
	...
	if ($eventDetail['event_class_id'] == event_CLASS_ID_RENGO) {
		...
		$webApplyDescription = '審査日の3ヶ月前';
		$webApplyStartDay = date('Y-m-d', strtotime($eventDetail['date_holder_grade']['result'][0]['event_date'] . ' -3 month'));
		$webApplyStartDayHtml = date('Y/m/d', strtotime($eventDetail['date_holder_grade']['result'][0]['event_date'] . ' -3 month'));
		$week = ['日', '月', '火', '水', '木', '金', '土'];
		$webApplyStartDayHtml .= '（' . $week[date('w', strtotime($webApplyStartDay))] . '）';
	} else {
		if (!empty($eventDetail['uketuke_limit_aikyuren_st'])) {
			$webApplyStartDay = date('Y-m-d', strtotime($eventDetail['uketuke_limit_aikyuren_st'] . ' -1 week'));
			$webApplyStartDayHtml = date('Y/m/d', strtotime($eventDetail['uketuke_limit_aikyuren_st'] . ' -1 week'));
			$week = ['日', '月', '火', '水', '木', '金', '土'];
			$webApplyStartDayHtml .= '（' . $week[date('w', strtotime($webApplyStartDay))] . '）';
		}
	}
	...
	$html .= '<dd>';
	$html .= form_file_document('event', 'event-files', $eventDetail['event_document_list'], $uploadFileNum);
	$html .= '</dd>';
	
	$html .= '</dl>';

	$html .= admin_form_hidden("event-id", $eventDetail['event_id']);
	$html .= admin_form_hidden("event-name", $eventName);
	$html .= admin_form_hidden("event-class-id", $eventDetail['event_class_id']);
 ...
 * return
 */
function form_event_regist_confirm($eventDetail, $eventDocumentList, $fileList) {
	...
	if ($eventDetail['event_class_id'] === event_CLASS_ID_CHUOU) {
		// 中央審査の場合は3日分の審査日を表示
		for ($i=1; $i<=3; $i++) {
			if (empty($eventDetail['event_date_' . $i]) === false) {
				$html .= $eventDetail['event_date_' . $i] . ' ' . $eventDetail['holder_grade_name_' . $i] . '<br>';
			}
		}
	} else {
		// 中央審査以外の場合は1日分の審査日を表示
		$html .= $eventDetail['event_date_1'] . ' ' . $eventDetail['holder_grade_name_1'] . '<br>';
		// $data = $eventDetail['date_holder_grade']['result'][0] ?? null;
		// if (empty($data) === false) {
		// 	$html .= '【' . $data['holder_grade_name'] . '】' . $data['event_date'];
		// }
	}
	$html .= '</dd>';
	// 審査種別総合（中央審査のみ）
	if ($eventDetail['event_class_id'] === event_CLASS_ID_CHUOU) {
		...
		$html .= '<dd>';
		$html .= $eventDetail['all_holder_grade_name'];
		$html .= '</dd>';
	}
	// 会場
	for ($i = 0; $i < 3; $i++) {
		if (empty($eventDetail['kaijo_id_' . ($i + 1)]) === false) {
			$html .= '<dt class="text-danger fw-bold">第' . ($i + 1) . '会場</dt>';
			$html .= '<dd>';
			$html .= $eventDetail['kaijo_name_' . ($i + 1)];
			if (empty($eventDetail['additional_info_' . ($i + 1)]) === false) {
				$html .= '<br>備考 : ' . $eventDetail['additional_info_' . ($i + 1)];
			}
			$html .= '</dd>';
		}
	}
	...
	$html .= '<dd>';
	// 全弓連締切日
	if (empty($eventDetail['uketuke_limit_zenkyuren']) === false) {
		$html .= '<dt class="text-danger fw-bold">全弓連締切日</dt>';
		$html .= '<dd>';
		$html .= date_format_jp($eventDetail['uketuke_limit_zenkyuren'], true, DATE_FORMAT_YMD);
		$html .= '</dd>';
	}
	// 愛弓連申込期間
	if (empty($eventDetail['uketuke_limit_aikyuren_st']) === false or empty($eventDetail['uketuke_limit_aikyuren_ed']) === false) {
		...
		$html .= '<dd>';
		if (empty($eventDetail['uketuke_limit_aikyuren_st']) === false) {
			// $html .= $eventDetail['uketuke_limit_aikyuren_st'];
			$html .= date_format_jp($eventDetail['uketuke_limit_aikyuren_st'], true, DATE_FORMAT_YMD);
		}
		$html .= ' ～ ';
		if (empty($eventDetail['uketuke_limit_aikyuren_ed']) === false) {
			// $html .= $eventDetail['uketuke_limit_aikyuren_ed'];
			$html .= date_format_jp($eventDetail['uketuke_limit_aikyuren_ed'], true, DATE_FORMAT_YMD);
		}
		$html .= '</dd>';
	}
	// WEB申込開始日
	if (empty($eventDetail['web_apply_start_day']) === false) {
		$html .= '<dt class="text-danger fw-bold">' . KASUGAI_KYOKAI_NAME_SHORT . '申込開始日</dt>';
		$html .= '<dd>';
		$html .= date_format_jp($eventDetail['web_apply_start_day'], true, DATE_FORMAT_YMD);
		$html .= '</dd>';
	}
	// 添付資料
	$html .= '<hr>';
	...
	// 登録済み添付資料
	$documentCnt = 0;
	if (empty($eventDocumentList['result']) === false) {
		$html .= '<section>';
		$html .= '<h3>[登録済み]</h3>';
		$html .= '<ul>';
		foreach ($eventDocumentList['result'] as $idy => $data) {
			$html .= '<li>';
