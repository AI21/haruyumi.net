...
// 審査リストHTML生成
function get_shinsa_tab_html($setting, $shinsaList)
{
    $html = '';
        ...
        foreach ($shinsaList as $tabName => $tabData) {
            ...
            if ($tabName == TAB_NAME_OTHER) {
                ...
                if ($tabData[DATA_MIRAI]['numRows'] > 0) {
                    ...
                    // $html .= '<caption>' . $setting->wareki . '(' . $setting->year . ')年度：未開催</caption>';
                    $html .= '<caption>未開催</caption>';
                    $html .= '<thead>';
                    $html .= '<tr>';
                    $html .= '<th>審査日</th>';
                    $html .= '<th>審査会名</th>';
                    ...
                    foreach ($tabData[DATA_MIRAI]['result'] as $idx => $data) {
                        $kaijoName = $data['kaijo_name_short'];
                        if ($data['kaijo_id'] === DB_FLG_OFF) {
                            $kaijoName = $data['kaijo_other_name_abb'];
                        }
                        $html .= '<tr>';
                        $html .= '<td>' . date_period_short_format($data['shinsa_date_min'], $data['shinsa_date_max']) . '</td>';
                        $html .= '<td><a href="./shinsa/detail/' . $data['shinsa_id'] . '">' . shinsa_name_short_html($data['area_group_name'], $data['shinsa_name']) . '</a></td>';
                    ...
                    $html .= '<div>登録されている審査はありません</div>';
                }
                if ($tabData[DATA_KAKO]['numRows'] > 0) {
                    $html .= '<table class="table caption-top shisa-list">';
                    // $html .= '<caption>' . $setting->wareki . '(' . $setting->year . ')年度：終了</caption>';
                    $html .= '<caption>終了</caption>';
                    $html .= '<thead>';
                    $html .= '<tr>';
                    $html .= '<th>審査日</th>';
                    $html .= '<th>審査会名</th>';
                    ...
                    foreach ($tabData[DATA_KAKO]['result'] as $idx => $data) {
                        $kaijoName = $data['kaijo_name_short'];
                        if ($data['kaijo_id'] === DB_FLG_OFF) {
                            $kaijoName = $data['kaijo_other_name'];
                        }
                        $html .= '<tr>';
                        $html .= '<td>' . date_period_short_format($data['shinsa_date_min'], $data['shinsa_date_max']) . '</td>';
                        $html .= '<td><a href="./shinsa/detail/' . $data['shinsa_id'] . '">' . shinsa_name_short_html($data['area_group_name'], $data['shinsa_name']) . '</a></td>';
...
// 中央審査詳細HTML生成
function get_shinsa_chuou_detail_html($shinsaDetail, $shinsaTarget, $memberGradeDeta)
{
    $html = '';
    $shinsaDateSt = '';
    $shinsaDateEd = '';
    ...
    if (empty($shinsaDetail) === false) {
        ...
        // 場所
        $kaijo = '未定';
        if (empty($shinsaDetail['kaijo_name']) === false) {
            $kaijo = $shinsaDetail['kaijo_name'];
        } else {
            // 会場マスタにない会場はその他会場を表示
            if (empty($shinsaDetail['kaijo_other_name']) === false) {
                $kaijo = $shinsaDetail['kaijo_other_name'];
            }
        }
...
// 連合審査詳細HTML生成
function get_shinsa_rengo_detail_html($shinsaDetail, $shinsaTarget, $memberGradeDeta)
{
    $html = '';
    $shinsaDateSt = '';
    $shinsaDateEd = '';
...
// 地方審査詳細HTML生成
function get_shinsa_chiho_detail_html($shinsaDetail, $shinsaTarget, $memberGradeDeta)
{
    $html = '';
    $shinsaDateSt = '';
    $shinsaDateEd = '';
